...
- Provides custom error types to give schema, shape, type, and range validation failures a consistent, identifiable error type across consuming code.
...
Custom error types extend native error types and expose a stable Node.js-style error code to give schema, shape, type, and range validation failures a consistent, identifiable error type for both TypeScript and JavaScript consumers.
